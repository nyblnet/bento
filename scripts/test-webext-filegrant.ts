...
console.log('\n— the file grant: proven, never assumed')
{
  ...
  const tw = world({ '/Users/andy/Downloads/T.bento.html': 'SAME', '/Users/andy/Documents/T.bento.html': 'SAME' })
  await fg.addFileGrant(handleOf('T.bento.html', 'SAME'), '/Users/andy/Downloads/T.bento.html', tw.deps)
  ok((await fg.resolveFileGrant('/Users/andy/Downloads/T.bento.html', tw.deps)).ok, 'a grant bound to a path serves that path')
  ok(!(await fg.resolveFileGrant('/Users/andy/Documents/T.bento.html', tw.deps)).ok, 'and NOT a byte-identical twin at another path')
  const un = world({ '/Users/andy/Downloads/U.bento.html': 'SAME', '/Users/andy/Documents/U.bento.html': 'SAME' })
  un.deps.otherCopies = async (p: string) => ['/Users/andy/Downloads/U.bento.html', '/Users/andy/Documents/U.bento.html'].filter((x) => x !== p)
  await fg.addFileGrant(handleOf('U.bento.html', 'SAME'), null, un.deps)
  ok(!(await fg.resolveFileGrant('/Users/andy/Downloads/U.bento.html', un.deps)).ok, 'an unbound grant is refused while a same-bytes twin exists (never guessed)')
  un.deps.otherCopies = async () => []
  ok((await fg.resolveFileGrant('/Users/andy/Downloads/U.bento.html', un.deps)).ok && [...un.store.values()].some((v) => v.path === '/Users/andy/Downloads/U.bento.html'), 'with no twin it serves the path and is bound to it from then on')
