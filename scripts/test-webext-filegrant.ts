...
function world(disk: Record<string, string>) {
  ...
  const deps: any = {
    ...
    downloads: {
      ...
      download: async (o: any) => { const id = nextId++; downloaded.push(o); items.set(id, { id, filename: `/Users/you/Downloads/${o.filename}`, state: 'complete' }); return id },
...
{
  const w = world({ '/Users/you/Downloads/Q3.bento.html': 'DECK-Q3', '/Users/you/Desktop/Q3.bento.html': 'OTHER' })
  const h = handleOf('Q3.bento.html', 'DECK-Q3')
  await fg.addFileGrant(h, null, w.deps)
  const r = await fg.resolveFileGrant('/Users/you/Downloads/Q3.bento.html', w.deps)
  ok(r.ok && r.handle === h, 'a name-keyed grant serves the path whose bytes it holds')
  ok([...w.store.values()].some((v) => v.path === '/Users/you/Downloads/Q3.bento.html'), 'and the path is learned onto the record')
  const other = await fg.resolveFileGrant('/Users/you/Desktop/Q3.bento.html', w.deps)
  ok(!other.ok && other.reason === 'none', 'the same name with other bytes is NOT served — a grant is for one file')
  ok(!(await fg.resolveFileGrant('/Users/you/Downloads/Nope.bento.html', w.deps)).ok, 'no grant of that name → none')
  // TWINS: a byte-identical copy elsewhere must never be served by the other's grant
  const tw = world({ '/Users/you/Downloads/T.bento.html': 'SAME', '/Users/you/Documents/T.bento.html': 'SAME' })
  await fg.addFileGrant(handleOf('T.bento.html', 'SAME'), '/Users/you/Downloads/T.bento.html', tw.deps)
  ok((await fg.resolveFileGrant('/Users/you/Downloads/T.bento.html', tw.deps)).ok, 'a grant bound to a path serves that path')
  ok(!(await fg.resolveFileGrant('/Users/you/Documents/T.bento.html', tw.deps)).ok, 'and NOT a byte-identical twin at another path')
  const un = world({ '/Users/you/Downloads/U.bento.html': 'SAME', '/Users/you/Documents/U.bento.html': 'SAME' })
  un.deps.otherCopies = async (p: string) => ['/Users/you/Downloads/U.bento.html', '/Users/you/Documents/U.bento.html'].filter((x) => x !== p)
  await fg.addFileGrant(handleOf('U.bento.html', 'SAME'), null, un.deps)
  ok(!(await fg.resolveFileGrant('/Users/you/Downloads/U.bento.html', un.deps)).ok, 'an unbound grant is refused while a same-bytes twin exists (never guessed)')
  un.deps.otherCopies = async () => []
  ok((await fg.resolveFileGrant('/Users/you/Downloads/U.bento.html', un.deps)).ok && [...un.store.values()].some((v) => v.path === '/Users/you/Downloads/U.bento.html'), 'with no twin it serves the path and is bound to it from then on')
  const lapsed = world({ '/Users/you/Downloads/L.bento.html': 'L' })
  await fg.addFileGrant(handleOf('L.bento.html', 'L', 'prompt'), '/Users/you/Downloads/L.bento.html', lapsed.deps)
  const lr = await fg.resolveFileGrant('/Users/you/Downloads/L.bento.html', lapsed.deps)
...
console.log('\n— downloads: the folder Chrome lets an extension write')
{
  ...
  ok(dir === '/Users/you/Downloads', `downloadsDir: learned from the API (${dir}) and`)
  ok(w.kv.downloadsDir === dir, 'remembered in extension storage')
  ok(fg.downloadsRelative('/Users/you/Downloads/Q3.bento.html', dir) === 'Q3.bento.html', 'downloadsRelative: a file in Downloads → its relative name')
  ok(fg.downloadsRelative('/Users/you/Downloads/decks/Q3.bento.html', dir) === 'decks/Q3.bento.html', 'downloadsRelative: nested is fine')
  ok(fg.downloadsRelative('/Users/you/Documents/Q3.bento.html', dir) === null && fg.downloadsRelative('/Users/you/Downloads/../x.html', dir) === null, 'downloadsRelative: outside, or climbing out → null')
  ...
  const win = world({}); win.deps.downloads.search = async () => [{ filename: 'C:\\Users\\you\\Downloads\\x.txt' }]
  ok(await fg.downloadsDir(win.deps) === '/C:/Users/you/Downloads', 'downloadsDir: a Windows path becomes the file:// shape')
}

console.log('\n— the worker: folder, then file, then Downloads; own file only')
const FILE = (p: string) => ({ url: `file://${p}`, frameId: 0 })
{
  const w = world({ '/Users/you/Downloads/Q3.bento.html': 'DECK-Q3' })
  const h = handleOf('Q3.bento.html', 'DECK-Q3')
  await fg.addFileGrant(h, '/Users/you/Downloads/Q3.bento.html', w.deps)
  const deps = { readGrants: async () => [], filegrant: w.deps }
  const c = await bg.claim(FILE('/Users/you/Downloads/Q3.bento.html'), deps)
  ok(c.ok && c.via === 'file' && c.name === 'Q3.bento.html', 'claim: no folder grant → the file grant serves it (via file)')
  const wr = await bg.write(FILE('/Users/you/Downloads/Q3.bento.html'), 'NEW', deps)
  ...
  const dl = await bg.claim(FILE('/Users/you/Downloads/Other.bento.html'), deps)
  ok(dl.ok && dl.via === 'downloads', 'claim: a Downloads file with no grant → the downloads API (via downloads)')
  const dw = await bg.write(FILE('/Users/you/Downloads/Other.bento.html'), 'BYTES', deps)
  ...
  const doc = await bg.claim(FILE('/Users/you/Documents/Q3.bento.html'), deps)
  ...
  const spoof = await bg.write({ url: 'file:///Users/you/Documents/x.bento.html', frameId: 0 } as any, 'X', { ...deps })
  ...
  const fc = await bg.claim(FILE('/Users/you/Downloads/Q3.bento.html'), folderDeps)
  ...
  const gone = world({ '/Users/you/Downloads/G.bento.html': 'G' })
  const gh: any = handleOf('G.bento.html', 'G')
  gh.createWritable = async () => { const e: any = new Error('gone'); e.name = 'NotFoundError'; throw e }
  await fg.addFileGrant(gh, '/Users/you/Downloads/G.bento.html', gone.deps)
  const gw = await bg.write(FILE('/Users/you/Downloads/G.bento.html'), 'X', { readGrants: async () => [], filegrant: gone.deps, downloadsDir: async () => null })
...
console.log('\n— the page side: waiting on the offer')
{
  ...
  const win: any = {
    location: { pathname: '/Users/you/Downloads/Q3.bento.html' },
    addEventListener() {},
    postMessage(msg: any) {
      posted.push(msg)
...
console.log('\n— a Chrome that asks where to save every download')
{
  ...
  const w = world({ '/Users/you/Downloads/P.bento.html': 'P' })
  w.deps.downloads.download = async (o: any) => { const id = 99; w.downloaded.push(o); return id }
  w.deps.downloads.search = async (q: any) => (q.id === 99 ? [{ id: 99, state: 'interrupted', error: 'USER_CANCELED', filename: '/Users/you/Downloads/P.bento.html' }] : [{ filename: '/Users/you/Downloads/x.txt' }])
  const deps = { readGrants: async () => [], filegrant: w.deps }
  const first = await bg.write(FILE('/Users/you/Downloads/P.bento.html'), 'X', deps)
  ...
  const next = await bg.claim(FILE('/Users/you/Downloads/P.bento.html'), deps)
