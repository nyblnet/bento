...
// gallery is an index, and three tour pages have no cover on purpose, so the
...
export function starterDoc(): SpacesDoc {
  ...
  return {
    ...
    pages: [
      ...
      // #tag onto eight pages of prose is for the tour to sit under it. That
      ...
      // paragraphs here are kept short for that reason, and the gap is filed.
      {
        ...
        blocks: [
          ...
          b('p', 'Eight pages, nested under this one and drawn as a gallery. Each demonstrates the thing it describes — the page about tables is one, the page about the board holds it — and every one is yours to rewrite or delete. Two pages in this space are left deliberately unfinished, and say so — one of these and the Inbox — because finishing them is how you meet the controls that do it. The picture on a card is the page’s <strong>cover</strong>, set from the page menu; a page without one gets a tint and its icon instead. The five cards on the tracker are under this page too, and are kept out of the gallery by one filter: a page with a status is work, and work has a board of its own.'),
          b('h2', 'Try it'),
      ...
      ],
      {
        ...
        blocks: [
          b('p', 'The same pages as the board next door, asked five different questions. None of these is a new kind of thing and none of them stores a copy of anything: a view is a question, answered from the pages themselves every time it is drawn. The last thing on this page is the exception, and it is placed last for that reason.'),
          ...
          // ── THE ROADMAP ────────────────────────────────────────────────────
          // Last, and after the two chart-shaped views, because it is the one
          // block on this page that is NOT derived: five views above it answer
          // questions from the pages, and this is a thing somebody wrote down.
          // Putting it between the views would break the sentence the page
          // opens with; putting it first would open a page called Planning on
          // the one part of it that plans nothing from the data. It is also
          // where the page stops looking back (last fortnight's schedule) and
          // looks forward, which is what a roadmap is for.
          b('h2', 'Where this is going'),
          roadmap,
          ...cards,
          b('p', 'Nothing above this heading was written; it was asked. This was written. A <strong>canvas</strong> is the one block where things sit where you put them: a roadmap, a storyboard, a map of an argument. Drag a card by its grip; <code>⏎</code> in a card makes another; <strong>Add a page</strong> makes a card that opens one, like the Inbox above. Positions are stored as a share of the surface rather than in pixels, so the same file lays out the same on a laptop, a monitor and a printout — and on a phone the surface gives up and becomes the list of what is on it.'),
      ...
      },
      {
        ...
        icon: 'graph',
        // Tour, so under Welcome and in the gallery (it has no status). No
        // cover — see Handing it over: the gallery needs cards without one,
        // and a drawing of a chart on a page of charts would be the brochure
        // the note at the top of this file warns against.
        parent: P.home,
        blocks: [
      ...
      },
      {
        ...
        icon: 'people',
        // NO COVER, on purpose, and the same for Sharing & limits and How it
        // is going: the gallery has to show what a page without a picture
        // looks like — a tint from the page id and the page's own icon — or
        // the starter would be claiming that every card needs one.
        parent: P.home,
        blocks: [
