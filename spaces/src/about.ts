...
export function openAbout(hooks: AboutHooks): void {
  const { store, onRepaint, onStatus, previewDesign, openDesignPanel } = hooks
  ...
  // (Password, the browser-local timeline, the JSON round trip, import and
  // the ways out are the document's own commands and live under Save ▾ now —
  // doccmds.ts. The in-file versions above stay here until the history
  // branch moves them into Version history…)
  ...
  // #559's About-close rule rides the kernel dialog's onClose: every way out
  // (Close, Escape, the scrim) ends a design hover-preview left running.
  dlg = createDialog({
