...
/**
 * Holds the Kotlin indexer to the SHARED corpus in `tray/fixtures/`.
 *
 * This is the whole mechanism that keeps three implementations of one algorithm
 * from drifting — see `tray/fixtures/README.md`. The reference is
 * `tray/doc-index.mjs`; `node scripts/test-doc-index.mjs` runs it against the
...
class DocumentIndexTest {
    ...
    private val fixtures: File by lazy {
        ...
        while (dir != null && !File(dir, "tray/fixtures/expected.json").exists()) dir = dir.parentFile
        requireNotNull(dir) { "could not locate tray/fixtures from ${System.getProperty("user.dir")}" }
        File(dir, "tray/fixtures")
    }
