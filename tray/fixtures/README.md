# tray/fixtures — the document-indexing contract
...
Three hosts answer them in three languages — `tray/webext` in JavaScript,
`tray/android` in Kotlin, `tray/ios` in Swift — because their list UIs are
...
how they are held together.** `tray/doc-index.mjs` is the reference,
...
| `tray/android` | `./gradlew :app:testDebugUnitTest` (JVM, no device needed) |
| `tray/ios` | not yet built |
...
recoverable — unlike the writable-stream semantics in `tray/bridge.js`, which
are shared outright because a divergence there wrote users' documents out as
zero bytes.

## Changing the algorithm

Change `tray/doc-index.mjs`, extend `cases/`, regenerate `expected.json`, then
